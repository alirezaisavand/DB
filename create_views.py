...
cur.execute('''CREATE VIEW Resrestaurant AS SELECT id, name, phoneNumber, area, type, minOrder from restaurant''')
cur.execute('''CREATE VIEW Resfood AS SELECT id, name, type, amount, description, price, restaurantId from food''')
cur.execute('''CREATE VIEW Resdelivery AS SELECT id,name,area,busy FROM delivery''')
cur.execute('''CREATE VIEW Rescustomer AS SELECT id,name,area,phoneNumber from customer''')
cur.execute('''CREATE VIEW Resorder AS SELECT id,restaurantId,preparingTime,customerId,orderTime from orderr''')
cur.execute('''CREATE VIEW Ressending AS SELECT orderId,deliveryId from sending''')
cur.execute('''CREATE VIEW ResfoodOrdered AS SELECT orderId,foodId from foodOrdered''')
